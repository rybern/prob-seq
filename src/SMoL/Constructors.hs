...
module SMoL.Constructors where
...
showConstructor (Symbol s) = "symbol (" ++ show s ++ ")"
...
symbol :: s -> ProbSeq s
symbol v = Fix $ Symbol v

symbols :: [s] -> ProbSeq s
symbols = series . map symbol
...
eitherOrM p a b = do
  t <- newTag [True, False]
  return (Fix $ EitherOr (Just (tagId t)) (max 0 . min 1 $ p) a b, t)

eitherOr :: Prob -> ProbSeq s -> ProbSeq s -> ProbSeq s
eitherOr p a b = Fix $ EitherOr Nothing (max 0 . min 1 $ p) a b
...
uniformDistRepeat' n a = Fix $ UniformDistRepeat' Nothing n a
  ...
  -- 2n states
finiteDistRepeatDirichletM :: [Double] -> ProbSeq s -> State TagGen (ProbSeq s, ([Double], Tag Int))
finiteDistRepeatDirichletM theta a = do
  let n = length theta
  let uniform = replicate n (1 / fromIntegral n)
  t <- newTag [1..n]
  return (Fix $ FiniteDistRepeatTerm (Just (tagId t)) a uniform a, (theta, t))
