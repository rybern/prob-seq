...
module SMoL.Matrix ( buildConstructor
                       , buildMatSeqTree
                       , compileSMoL
                       , module SMoL.Matrix.Types ) where

import SMoL.Constructors
import SMoL.Matrix.Types
import SMoL.Matrix.ProbSeqMatrixUtils
import qualified SMoL.Matrix.Operations as Ops
import Data.Fix
...
compileSMoL :: (Eq s) => ProbSeq s -> MatSeq s
compileSMoL = Ops.filterUnreachableStates . cata (buildConstructor $| parTraversable rpar)
...
buildConstructor (Symbol s) = Ops.state s
